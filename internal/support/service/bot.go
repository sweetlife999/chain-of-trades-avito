...
// BotNickname обязан совпадать с ником из миграции 00024: запрос находит служебного
// пользователя джойном по нику, поэтому UUID в Go не хранится. Разъехавшись, эти два
// места дают тихое молчание бота, а не ошибку, — поэтому совпадение проверяется на старте
// (см. newSupportBot в cmd/api).
//
// Ник виден пользователю над сообщением, и это имя маскота: одной буквой «У» назвать
// нельзя, users держит CHECK (char_length(nickname) BETWEEN 3 AND 32).
const BotNickname = "У-бот"
...
// придёт человек.
const botDisclaimer = "Привет! Я У. Я могу ошибаться, поэтому если проблема не будет решена, напишите, и я позову модератора.\n\n"
